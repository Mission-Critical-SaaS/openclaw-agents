...
- **Primary tools**: Jira (via mcporter), Zendesk (via mcporter)
...
- **Primary tools**: Jira (via mcporter)
...
- **Secondary tools**: Jira (via mcporter, limited)
...
| Tool | Scout | Trak | Kit |
|------|-------|------|-----|
| **Jira** (mcporter) | ✅ Full — ticket lookup, creation, search | ✅ Full (primary) — sprint tracking, issue management | ⚠️ Limited — issue lookup only |
| **Zendesk** (mcporter) | ✅ Full — ticket search, create, update, comments | ❌ Not configured | ❌ Not configured |
| **Notion** (mcporter) | ❌ Not configured | ❌ Not configured | ❌ Not configured |
| **GitHub** (gh CLI) | ⚠️ Limited — issue search only | ✅ Full — PRs, issues, repos | ✅ Full (primary) — PRs, CI, code review |

**Note**: Notion is available in mcporter but not yet assigned to any agent's identity. All three agents CAN access it if their IDENTITY.md is updated.
...
### 1. mcporter Level
All MCP servers (Jira, Zendesk, Notion) are available to all agents via the shared mcporter config at `/root/.mcporter/mcporter.json`. This is generated by the inner entrypoint.

### 2. Agent Identity Level
Each agent's `IDENTITY.md` file defines which tools the agent should USE. The gateway doesn't restrict tool access — agents self-select based on their identity instructions. An agent without Zendesk in its IDENTITY.md can technically call `mcporter call zendesk.*` but won't know to do so.
...
### GitHub Tools (via gh CLI, not mcporter)
...
```bash
# On EC2, verify mcporter sees all servers
docker exec openclaw-agents bash -c "HOME=/root mcporter list --json"
# Expected: 3 servers, all status "ok"
...
| Watchdog Channel | #openclaw-watchdog (C0AL58T8QMN) — private |
