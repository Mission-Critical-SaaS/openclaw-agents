...
When MCP servers fail with `Cannot find module 'ajv'` (or other missing module errors), the failure appears in logs as `MCP error -32000: Connection closed`. The underlying stack trace shows broken file paths like `/root/.npm/_npx/764ca7ecbcc84da3/node_modules/ajv-formats/dist/limit.js`.
...
The MCP server configuration file is located at `/root/.mcporter/mcporter.json` inside the container. This config is generated by the inner entrypoint (`docker/entrypoint.sh`) on each container start and is ephemeral (lost when the container stops).
