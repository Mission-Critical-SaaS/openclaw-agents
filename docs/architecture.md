...
1. **MCP server configuration**: Generates `/root/.mcporter/mcporter.json` with Jira, Zendesk, and Notion server endpoints from env vars. mcporter is the MCP server manager used internally by the OpenClaw gateway.
...
Agent workspace files are stored in `/opt/openclaw-persist/agents/{agent}/workspace/` on the host, bind-mounted directly to `/root/.openclaw/agents/{agent}/workspace/` inside the container. This means agent writes (memory updates, bootstrap state changes) go straight to persistent host storage.
...
- `/root/.mcporter/mcporter.json` — MCP server configuration (regenerated by inner entrypoint)
