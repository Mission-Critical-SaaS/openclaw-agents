...
OpenClaw Agents runs as a single Docker container on an EC2 instance. The container runs the `openclaw` npm package in gateway mode, which manages five Slack bot agents (Scout, Trak, Kit, Scribe, Probe) through a unified configuration. The container runs as a non-root `openclaw` user (UID 1000) via gosu, with bind-mount permission fixups at startup.
...
8. **Agent bootstrap**: Sends a lightweight agent turn (`openclaw agent --agent main`) to force-load all MCP servers, eliminating cold-start latency
...
1. **MCP server configuration**: Generates `/home/openclaw/.mcporter/mcporter.json` with Jira, Zendesk, and Notion server endpoints from env vars. mcporter is the MCP server manager used internally by the OpenClaw gateway.
2. **Agent auth profiles**: Creates agent-specific auth profiles for all five agents (stored in `/home/openclaw/.mcporter/`)
...
#### Persistent Agent Workspace (`/opt/openclaw-persist/`)

Agent workspace files are stored in `/opt/openclaw-persist/agents/{agent}/workspace/` on the host, bind-mounted directly to `/home/openclaw/.openclaw/agents/{agent}/workspace/` inside the container. This means agent writes (memory updates, bootstrap state changes) go straight to persistent host storage.

The inner entrypoint (`docker/entrypoint.sh`) uses selective copy logic on startup:

- **IDENTITY.md**: Always overwritten from the git-managed source (`/opt/openclaw/agents/{agent}/workspace/`). Deploy changes to agent instructions take effect on next container start.
- **KNOWLEDGE.md**: Seeded from git only if it doesn't already exist in the persistent directory. Runtime edits (agent memories) survive across restarts and deploys.
- **BOOTSTRAP.md**: Seeded from git only if it doesn't already exist. Once an agent deletes it (marking itself as bootstrapped), it stays deleted across restarts and deploys.

The persistent directory lives outside the git repository (`/opt/openclaw-persist/` vs `/opt/openclaw/`), so `git checkout` during deploys never touches agent runtime state.
...
- `/home/openclaw/.mcporter/mcporter.json` — MCP server configuration (regenerated by inner entrypoint)
- `/home/openclaw/.openclaw/openclaw.json` — OpenClaw gateway configuration (regenerated by gateway process)
- `/home/openclaw/.npm/_npx/` — npx package cache (rebuilt from scratch on each startup)

#### Key Implication: Configuration Persistence

**Changes made via `openclaw config set` are LOST on container restart.** The `openclaw config set` command modifies `/home/openclaw/.openclaw/openclaw.json`, which is regenerated from scratch on the next restart.
...
Each agent has a different set of tools and primary responsibilities. The OpenClaw gateway makes all MCP tools available to all agents via a shared configuration. Agents self-select which tools to use based on their IDENTITY.md instructions — the gateway does not restrict access per agent.

| Agent | Jira (MCP) | Zendesk (MCP) | Notion (MCP) | GitHub (gh CLI) | Primary Role |
|-------|-----------|---------------|--------------|-----------------|-------------|
...
| Scribe | ⚠️ Limited | ❌ | ✅ Full (primary) | ❌ | Knowledge/Docs |
| Probe | ⚠️ Limited | ❌ | ❌ | ✅ Full | Quality/Testing |

**Notes:**
- Tool access is defined in each agent's IDENTITY.md, not in gateway config
- The gateway makes all MCP tools available; agents self-select based on their identity instructions
...
- **CloudWatch Alarms**: CPU, status checks, and disk monitoring via CDK-defined alarms
- **Health check**: Message any agent via DM and verify response
- **MCP server status**: `docker exec openclaw-agents openclaw status` (via SSM)

## Deployment

All deployments go through GitHub Actions CI/CD. Pushing a version tag triggers the pipeline:

1. **Test gate**: `npx jest` runs unit and infrastructure tests — deploy is blocked if tests fail
2. **Deploy via SSM**: Sends commands to the EC2 instance to pull, build, and restart
3. **Verify**: Checks docker logs for bootstrap success and Slack connections
4. **Notify**: Posts to Slack on failure

See [deploy.md](playbooks/deploy.md) for full deployment procedures.

## Known Issues and Gotchas

### npx Cache Corruption

The npx cache at `/home/openclaw/.npm/_npx/` can become corrupted, especially after abrupt container shutdowns. Corruption causes MCP servers to go offline mid-session without error messages in the gateway logs. If agents suddenly lose tool access:

1. Check `/home/openclaw/.npm/_npx/` for stale or incomplete package installations
...
Each agent caches the list of available tools when it starts up. If you fix an MCP server (e.g., clear a corrupted npx cache), the agent will not see the restored tools until the container restarts. Changes to tool availability require a full container restart, not just a gateway config reload.

### SIGHUP Causes Process Exit, Not Graceful Reload

Sending SIGHUP to the gateway PID (the main OpenClaw process) causes the entire process to exit with code 129, triggering a full container restart. This is not a graceful config reload — it's a complete process restart. To apply config changes without SIGHUP:
...
When you run `docker exec openclaw-agents bash` and test MCP tools, you may get different results than what agents see. This is because `docker exec` uses a different shell environment than the agent processes spawned by the entrypoint. Agent processes inherit the full env vars and auth profiles from the entrypoints; interactive shells do not.
